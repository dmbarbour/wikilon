...
// slice of LMDB API used for Stowage
module LMDB =
    type MDB_env = unativeint
    type MDB_txn = unativeint
    type MDB_dbi = uint32
    type MDB_cursor = unativeint
    type mdb_mode_t = int
    ...
    // API methods
    //
    // Note: Using byte[] for strings (I'll convert explicitly).
