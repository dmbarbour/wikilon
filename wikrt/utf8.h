...
/** Compute contiguous sequence of valid UTF-8 up to given limit. */
static inline size_t utf8_strlen(uint8_t const*s, size_t const maxlen)
{
    size_t rem = maxlen;
    uint32_t cp;
    do { /* nop */ } while(utf8_step(&s, &rem, &cp));
    return (maxlen - rem); 
}
