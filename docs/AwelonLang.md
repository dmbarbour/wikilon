...
The simplest parallelism that clients might request is parallel evaluation of a block via `[computation](par)`. The computation block can be moved around linearly while computing. We can further accelerate composition involving parallel computations such that `[arg]request` outputs generated by one computation are applied to the next in an adjacent structure without waiting for a complete return.

Unfortunately, `(par)` has severe limitations on expressiveness. It is at once too expressive to leverage low-level parallelism (vector processing, SIMD, SSE, GPGPU) and insufficient to express more general connectivity models than basic adjacency.

For low level parallelism, we might accelerate [linear algebra](https://en.wikipedia.org/wiki/Linear_algebra). Alternatively, we accelerate evaluation for a safe subset of OpenCL or WebCL or some Khronos group spec, with which we later accelerate linear algebra. Either approach can get Awelon into machine learning, physics simulations, graphics and audio, and other high performance computing systems.
