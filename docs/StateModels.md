...
Users can ascertain authority code and a little information about depth, but nothing about location in tree. The capability provides all the authority necessary to use it. So, while it's somewhat large, it's also combining all the roles of user (via tracker) and password (via HMAC), and may be used in ways that normal URLs cannot. Embedding in bytecode would generally take the form:

        "Mdpxgznkbmxftkjdphhgqsmbszdzmtnkypxqhzxqmzstsqmszqybtqskd
        ~{&rsc}[{:rsc}]vr$c

The annotation `{&rsc}` would assert that this is a valid identifier. This little step would mitigate many issues that arise from embedding strings and links in code! For example, we could easily scan for broken links. It may also enhance performance if we plan to use the resource many times. (Some of this might be obfuscated by foreign resource names; it may be useful to address distribution concerns early on.) The `{:rsc}` discretionary sealer then prevents us from directly touching the string (at least by accident). 
