...
Awelon Object (AO) language uses [Awelon Bytecode (ABC)](AboutABC.md) as a foundation for programming, introducing two major features:

* ability to link between **words** within a **dictionary**
* support for structured editing and EDSLs within a word

Words provide the basis of modularity and linking. The dictionary replaces conventional models for sharing and packaging code. Support for structured editing and DSLs arises from how word definitions are typed.
...
Dictionaries are always *complete*. There are no external dependencies. There is no need for separate packages or libraries. 

Open source communities can create and curate very large, general purpose dictionaries with millions of words including the little things like clipart, fonts, and game music. Developers and software companies can easily fork an appropriately licensed open source dictionary and have the full power of the community at their fingertips. DVCS-like mechanisms - forking, merging, patching, cherry-picking, pull-requests - allow sharing between active dictionaries. These concepts replace package managers.

Conversely, when sharing a project, it is trivial to pare a dictionary down to just the essential root words and transitive dependencies. Small dictionaries can be useful for distribution or didactic reasons.

*ASIDE:* Interestingly, dictionaries might be a useful basis for genetic programming, treating a dictionary as DNA and words as genes, and using a probabilistic merge.
...
AO does not use the conventional libraries/packages distribution model. A dictionary never has any external dependencies.


 Dependencies between words must be acyclic, and dictionaries must always be *complet . A very simple import/export file format is specified:


Dictionaries should be acyclic. To help enforce this structurally, our export format should always define all dependencies for a word before defining the word. E.g. `swapd` is defined after `swap`.



 and have a purely functional meaning. AO code is restricted in some important. Tokens in AO definitions are limited to words, annotations, and discretionary sealers/unsealers. Words cannot be used when computing the definition. Dependencies between words must be acyclic. 

 computes and renders this value, supports direct manipulations of it, treating the value as a syntax tree. The additional function `[v→[a→b]]` becomes language descriptor, and will frequently be a block containing another word, e.g. `[{%fooLang}]`. In the trivial case, we may compute an identity.

  the language may even be identity, meaning the definition directly computes  
...
*ASIDE:* An older version of AO provided a thin syntactic layer above the bytecode. However, this has been dropped because it hinders development of structured editors and DSLs.
...
AO has gone through one major iterat
