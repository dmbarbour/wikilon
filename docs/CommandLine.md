...
A claw block contains more claw code. But a relevant question is: where does the block go after we introduce it? At the bytecode layer, a block is added to the environment as the first element in a pair, e.g. `e → (block*e)`. But in a Claw environment, we'll want blocks to be placed onto a stack (e.g. `(stack*env) → ((block*stack)*env)`) or some other appropriate region. 

For text and numbers, the words `integer` and `literal` and so on are responsible for moving the block to an appropriate location. So I think I should do the same here, perhaps something like:

        [foo]   desugars to     \[foo] block

The `\[foo]` block is then an 'escaped' block form. This represents any blocks that are not immediately processed by a following `block` word, e.g. in context of round-tripping.  An escaped block is still presented as containing Claw code. 
...
A weakness of namespaces is that they do hurt refactoring. If working with Claw code, developers must be careful to not move code into a different namespace. We could do this via copy-paste at the AO/ABC layer instead of at the Claw layer, and perhaps record the namespace with every selection. Operating at the AO or ABC layers doesn't have any issues with context.

### Claw Semantics and Round Tripping

From the points above, claw code is essentially a simple syntactic sugar above AO. The entire semantics of claw code is its trivial expansion into AO. The inverse is possible to support round-tripping: a parser can recover structure and present it to the user for editing. The main weakness is that such a parser will be oriented around keywords (integer, literal, block, ratio, decimal, exp10). We cannot recover useful structure from arbitrary AO code. I believe this is an acceptable limitation.

This simple design meets my requirements, and is reasonably extensible.
